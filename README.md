#### `Class quasi-template` is very similar to `class template`, but it's syntax is different
...
Here is described another type of `class` which fits this definition and since it is very similar to `class template` 
but is not exactly the same, it is called `class quasi-template`.
